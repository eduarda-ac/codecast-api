...
public class StudioService {
    ...
    public List<StudioResponseDTO> findAll(){
        return studioRepository.findAll().stream().map(this::toResponseDTO).collect(Collectors.toList());
    }
    public StudioResponseDTO findById(Long id){
    Studio studio = studioRepository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Estudio nao encontrado: " + id));
    ...
    public void delete(Long id){
        if(!studioRepository.existsById(id)){
            throw new ResourceNotFoundException("Estudio não encontrado: " + id);
